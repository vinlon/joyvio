myFocus.pattern.extend({//*********************pithy******************
    'mF_pithy_tb': function (settings, $) {
        ...
        var p = settings,
			...
			thuHeight = Math.round(thuBoxHeight / showNum - 5),
			n = $txtList.length;

        $txtList.each(function () {
            if ($(this).find('p').html() == '') {
                $(this).find('b').css({ height: 0 });
            }
        });

        $focus[0].style.width = p.width + p.thumbWidth + 'px';
        $picList.each(function () { this.style.height = p.height + 'px' });
        $thumbBox.css({ width: p.thumbWidth, height: thuBoxHeight - 18, top: p.thumbBtnHeight });
        ...
        $prevBtn2.css({ left: p.width + 10 });
        $nextBtn2.css({ left: p.width + 10 });
        //PLAY
